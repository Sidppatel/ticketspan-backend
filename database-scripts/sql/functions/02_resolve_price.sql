...
AS $$
DECLARE
    v_tenant uuid; v_type text; v_base int; v_per int; v_allinc bool; v_explicit uuid;
    v_formula uuid; v_seats int := GREATEST(COALESCE(p_seats, 1), 1);
    v_rule_price int; v_unit int; v_sub int; v_fee int;
BEGIN
    SELECT tenants_id, pricing_type, base_price_cents, per_attendee_cents,
           is_all_inclusive, fee_formulas_id
      INTO v_tenant, v_type, v_base, v_per, v_allinc, v_explicit
      FROM prices
    ...
    SELECT pr.price_cents INTO v_rule_price
      FROM price_rules pr
     WHERE pr.prices_id = p_prices_id
