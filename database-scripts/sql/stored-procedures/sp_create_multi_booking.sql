...
-- Pricing is fully server-authoritative (app.resolve_price per line); the client
-- never supplies amounts. Table lines reserve the WHOLE table (seats = capacity)
-- and lock the table row atomically for the hold window. The whole cart succeeds
-- or fails together, so a table grabbed by someone else mid-cart rolls back the
-- entire reservation.
--
...
DECLARE
    ...
    v_sub int := 0; v_fee int := 0; v_total int := 0;
    v_line jsonb; v_kind text; v_ref uuid; v_seats int;
    v_prices_id uuid; v_unit int; v_formula uuid;
    v_ls int; v_lf int; v_lt int;
...
BEGIN
    ...
    FOR v_line IN SELECT * FROM jsonb_array_elements(p_lines) LOOP
        ...
        IF v_kind = 'Ticket' THEN
            ...
            SELECT prices_id, price_cents, fee_formulas_id, max_quantity
              INTO v_prices_id, v_unit, v_formula, v_tt_max
            ...
            -- Per-tier cap: live seats already taken (both booking models) + this line.
            ...
            IF v_prices_id IS NOT NULL THEN
                SELECT subtotal_cents, fee_cents, total_cents INTO v_ls, v_lf, v_lt
                  FROM app.resolve_price(v_prices_id, now(), v_seats,
                                         app.remaining_for_price(v_prices_id));
            ELSE
                v_ls := v_unit * v_seats;
                v_lf := app.compute_fee(v_unit, v_formula) * v_seats;
                v_lt := v_ls + v_lf;
            END IF;
            ...
            v_resolved := v_resolved || jsonb_build_object(
                ...
                'subtotal', v_ls, 'fee', v_lf, 'total', v_lt);
        ...
        ELSIF v_kind = 'Table' THEN
            ...
            SELECT t.status, t.locked_by_users_id, t.lock_expires_at,
                   ...
                   et.prices_id, et.price_cents, et.fee_formulas_id
              INTO v_tbl_status, v_locked_by, v_lock_exp, v_cap, v_prices_id, v_unit, v_formula
            ...
            END IF;
            -- A live lock owned by another user blocks; the buyer's own stale/live
            -- lock is fine to take over.
            ...
            ELSE
                v_ls := v_unit;
                v_lf := app.compute_fee(v_unit, v_formula);
                v_lt := v_ls + v_lf;
            END IF;
    ...
    -- Create the aggregate booking. Single-line columns stay NULL; the lines carry
    -- the detail and the booking totals are the cart sum.
    LOOP
        ...
        BEGIN
            INSERT INTO bookings (tenants_id, booking_number, status, users_id, events_id,
                tables_id, seats_reserved, event_ticket_types_id,
                subtotal_cents, fee_cents, total_cents, hold_expires_at, created_at, updated_at)
            VALUES (v_tenant, v_number, 'Pending', p_user_id, p_event_id,
                NULL, NULL, NULL, v_sub, v_fee, v_total,
    ...
    -- Pass 2: persist lines + lock table rows for the hold window.
    FOR v_line IN SELECT * FROM jsonb_array_elements(v_resolved) LOOP
        INSERT INTO booking_lines (tenants_id, bookings_id, kind, event_ticket_types_id,
            tables_id, prices_id, seats, subtotal_cents, fee_cents, total_cents, created_at, updated_at)
        VALUES (v_tenant, v_id, v_line->>'kind',
            (v_line->>'ett')::uuid, (v_line->>'tbl')::uuid, (v_line->>'prices_id')::uuid,
            (v_line->>'seats')::int, (v_line->>'subtotal')::int, (v_line->>'fee')::int,
            (v_line->>'total')::int, now(), now());

        IF v_line->>'kind' = 'Table' THEN
            INSERT INTO booking_tables (tenants_id, bookings_id, tables_id)
            VALUES (v_tenant, v_id, (v_line->>'tbl')::uuid);
