...
public sealed partial class BookingServiceImpl : BookingService.BookingServiceBase
{
    ...
    private const string BookingSelect =
        ...
        + "COALESCE(b.user_email, ''), (COALESCE(b.user_first_name, '') || ' ' || COALESCE(b.user_last_name, '')) "
        + "FROM vw_bookings b";
    ...
    private static Booking MapBooking(NpgsqlDataReader r) => new()
    {
        ...
        UserEmail = r.IsDBNull(27) ? string.Empty : r.GetString(27),
        UserName = r.IsDBNull(28) ? string.Empty : r.GetString(28).Trim()
    };
